...
const Controller = {
  ...
  requestDeleteProfile: async (req, res) => {
    const { email } = req.body;
    try {
      const user = await User.findOne({ email });
      if (!user) return jsonFailed(res, null, "Account has been deleted");
      const otp = Math.floor(100000 + Math.random() * 900000).toString();
      await User.updateOne({ otp });
      const subject = "Delete Account";
      const text = `You requested to delete your account. Kindly use the otp ${otp} to complete your request. Kindly note that this will expire in 30 minutes. Cheers!`;
      SendEmail(user, subject, resendTemplate(user.firstName, text));
  ...
  deleteProfile: async (req, res) => {
    ...
    try {
      const user = await User.findOne({ email });
      if (!user) return jsonFailed(res, null, "Account has been deleted");
      if (user.otp !== otp) return jsonFailed(res, null, "Invalid OTP sent");
  ...
  batchLookup: async (req, res) => {
    try {
      const ids = Array.from(new Set((req.body.ids || []).map(String))).filter(
        Boolean,
      );
